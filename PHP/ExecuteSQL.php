...
function executeAllSQL($pdo) {
    executeSQL($pdo, "CREATE TABLE IF NOT EXISTS translations (
        ...
        lang VARCHAR(255),
        Title VARCHAR(255),
        language_button VARCHAR(255),
        email VARCHAR(255),
        score VARCHAR(255),
        link_email_score_button VARCHAR(255))");
...
function insertTranslationsSQL($pdo, $translationsTable) {
    executeSQL($pdo, "INSERT INTO $translationsTable
        VALUES (1, 'English', 'Trivia Space', 'See a random question', 'Your email:', 'Your score', 'Link email and score'),
